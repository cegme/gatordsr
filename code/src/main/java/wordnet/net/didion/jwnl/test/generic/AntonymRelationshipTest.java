	...
	public void testSimpleSenseKey() {
		try {
			...
			if (beaSynset != null && uglySynset != null) {
				RelationshipList list = RelationshipFinder.getInstance().findRelationships(beaSynset, uglySynset,
						PointerType.ANTONYM);
				...
				while (i.hasNext()) {
					...
					/**
					 * There should be only ONE relationship found at this point, and it should match the ugly
					 * synset.
					 */
